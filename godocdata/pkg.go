...
// jsonDoc prints all the docs as JSON
func (pkg *Package) jsonDoc() {
	pkg.buf.printed = true // Prevent package clause.
	data := make(map[string]any)
	data["name"] = pkg.name
	data["importPath"] = pkg.build.ImportPath
	...
	if err != nil {
		pkg.Fatalf(err.Error())
	}
	fmt.Fprintf(&pkg.buf, string(text))
}
